...
export default function createUtils({ fs = nodeFs, cp = nodeCp } = {}) {
  ...
  // const createGitTag = ({ version, customTag, dryRun, message = '' } = {}) => {
  //   if (!customTag && !version) {
  //     throw new Error('createGitTag requires either a version or customTag');
  //   }
  //   const tag = customTag || `v${version}`;

  //   if (dryRun) {
  //     console.log(
  //       chalk.blue(`Dry run, would have added and pushed tag: ${tag}`)
  //     );
  //     return;
  //   }

  //   cp.execSync(`git tag -a ${tag} -m "${message || tag}"`);
  //   cp.execSync('git push --tags');
  //   console.log(chalk.green(`Added git tag ${tag}!`));
  // };

  const createGitRelease = async ({ version, dryRun }) => {
    if (dryRun) {
      console.log(chalk.blue('Releasing to GitHub (dry run)'));
      return;
    }
    try {
      console.log('Releasing to GitHub...');
      await axios.post(
        ...
        },
        {
          headers: {
            ...
            Authorization: `Bearer ${process.env.GIT_RELEASE_TOKEN}`,
          },
        }
      );
      ...
      // console.error(e);
      console.error('Headers:', e.request._header);
