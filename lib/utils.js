...
export default function createUtils({
  ...
  const getGitHubInfo = (changelog) => {
    const changelogEntries = changelog.split(/##.*\n/);
    const body = changelogEntries[1].trim();
    // TODO: Maybe get the title from the changelog too
    return { body };
  };

  const createGitRelease = async ({ version, body, dryRun } = {}) => {
    if (!version) {
      throw new Error('Version is required');
    }

    const tag = `v${version}`;
    // TODO: Maybe add title to name
