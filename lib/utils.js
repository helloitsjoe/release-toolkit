...
export default function createUtils({ fs = nodeFs, cp = nodeCp } = {}) {
  ...
  const getPreviousVersion = () => {
    const previousPackageJson = cp
      .execSync(
        `git fetch origin && git show origin/${getDefaultGitBranch()}:package.json`
      )
      .toString();
  ...
  const updateVersion = async (projectRoot, newVersion) => {
    ...
    try {
      packageLockExists = await fs.access(
  ...
  const createGitTag = ({ version, customTag, message = '' } = {}) => {
    if (!customTag && !version) {
      throw new Error('createGitTag requires either a version or customTag');
    }
    const tag = customTag || `v${version}`;
    cp.execSync(`git tag -a ${tag} -m "${message || tag}"`.trim());
    console.log(
      chalk.green(`Added git tag ${tag}!
  To push with tags, run \`git push --follow-tags\`
  Or to undo, run \`git tag -d ${tag}\``)
    );
  };

  const getGitTags = () => {
    return cp.execSync('git tag').toString().split('\n').filter(Boolean);
  };
