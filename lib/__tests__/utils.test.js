...
`);
  ...
  describe('getRepo', () => {
    ...
    it('getRepo gets GitHub repo name (ssh)', () => {
      cp.execSync.mockReturnValueOnce(
        'git@github.com:helloitsjoe/release-toolkit.git'
      );
      const { getRepo } = createUtils({ cp });
      const repoName = getRepo();
      expect(repoName).toBe('helloitsjoe/release-toolkit');
    ...
    it('getRepo gets GitHub repo name (https)', () => {
      cp.execSync.mockReturnValueOnce(
        'https://github.com/helloitsjoe/release-toolkit.git'
      );
      const { getRepo } = createUtils({ cp });
      const repoName = getRepo();
      expect(repoName).toBe('helloitsjoe/release-toolkit');
...
`
  ...
  describe('git release', () => {
    it('creates release from version', async () => {
      const body = { tag_name: 'v1.2.3', name: 'v1.2.3', body: 'v1.2.3' };
      const headers = {
      ...
      expect(axios.post).toBeCalledWith(GIT_RELEASE_URL, body, { headers });
    });
