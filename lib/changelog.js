...
const changelog = async ({
  ...
  const existingChangelog = await utils.getChangelog(projectRoot);
  const version = await utils.getCurrentVersion(projectRoot);
  ...
  const newChanges =
    `## [${version}](https://github.com/${utils.getRepo()}/releases/tag/v${version}) (${utils.getDate()})
...
${existingChangelog}`.trim() + '\n';
  ...
  await utils.updateChangelog(projectRoot, newChanges);
